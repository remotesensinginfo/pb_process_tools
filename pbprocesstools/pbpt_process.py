...
class PBPTProcessTool(PBPTProcessToolsBase):
    ...
    def __init__(self, cmd_name=None, descript=None, params=None, uid_len=6):
        ...
        :param params: optionally provide a dict which will be the options for the
                       processing to execute (e.g., the input and output files).
    ...
    def outputs_present(self, **kwargs):
        ...
        outputs as keys and error message as the value). The boolean relates to whether
        all the outputs are present on the system; True all are present and the dict
        will be empty, False and some of the outputs will not be present and an error
    ...
    def std_run(self, **kwargs):
        ...
        :param kwargs: allows the user to pass custom variables to the function
                       (e.q., obj.std_run(var='blah')). These options are passed
                       to the parse_cmds(), do_processing() and completed_processing()
                       functions.
...
class PBPTGenProcessToolCmds(PBPTProcessToolsBase):
    def __init__(
        ...
        :param cmds_sh_file: the output file with the list of the commands to be
                             executed (e.g., /file/path/cmds_list.sh).
        :param out_cmds_base: the base output file name and path for the individual
                              commands (e.g., /file/path/sgl_cmd_).
        :param confirm_exe_dir: an output directory within which files will be created
                                to confirm that processing has completed. If a job
                                completion file is present it can be assumed that the
                                job has full completed (i.e., an exception has not
                                been thrown).
    ...
    def check_job_outputs(
        ...
        **kwargs,
        ...
        :param process_tools_cls: the name of the class implementing the
                                  ...
                                  checked.
        :param out_err_file: the output file name and path for the output error report
                             from this function where output files have not all be
                             created.
        :param out_non_comp_file: the output file name and path for the output error
                                  report from this function where processing might
                                  not have fully completed.
        :param cmd: optional input to override the __init__ variable. The command to
                    be executed (e.g., python run_analysis.py).
        :param cmds_sh_file: optional input to override the __init__ variable. The
                             output file with the list of the commands to be executed
                             (e.g., /file/path/cmds_list.sh).
        :param out_cmds_base: optional input to override the __init__ variable. the
                              base output file name and path for the individual
                              commands (e.g., /file/path/sgl_cmd_).
        :param kwargs: allows the user to pass custom variables to the function
        ...
        if out_cmds_base is None:
            ...
            out_cmds_base = os.path.join(
                cmds_sh_file_path, "{}_cmd_".format(cmds_sh_file_basename)
            )
    ...
    def write_cmd_files(
        self, cmd=None, cmds_sh_file=None, out_cmds_base=None, confirm_exe_dir=None
    ):
        """
        A function to write the output files for the commands.

        :param cmd: optional input to override the __init__ variable. The command to be
                    executed (e.g., python run_analysis.py).
        :param cmds_sh_file: optional input to override the __init__ variable. The
                             output file with the list of  the commands to be executed
                             (e.g., /file/path/cmds_list.sh).
        :param out_cmds_base: optional input to override the __init__ variable. the
                              base output file name and path for the individual
                              commands (e.g., /file/path/sgl_cmd_).
        :param confirm_exe_dir: optional input to override the __init__ variable.
                                an output directory within which files will be created
                                to confirm that processing has completed.
        ...
        with open(cmds_sh_file, "w") as cmds_sh_file_obj:
            for i, param in enumerate(self.params):
                ...
                param["confirm_exe"] = os.path.join(
                    confirm_exe_dir, "{}.ref".format(job_basename)
                )
    ...
    def create_slurm_sub_sh(
        ...
        mem_per_core_mb,
        log_dirs,
        run_script="gen_exe_hpccmds.sh",
        sbatch_config="config-sbatch.json",
        sbatch_template="sbatch_template.jinja2",
        account_name=None,
        n_cores_per_job=10,
        n_jobs=10,
        job_time_limit="2-23:59",
        sub_scripts_dir="./subscriptsplit",
        prepend=None,
        cmds_sh_file=None,
        ...
        :param sbatch_config: the file name and path for the config json file for
                              the sbatch submission script.
        :param sbatch_template: the file name and path for the jinja2 template for
                                the sbatch submission script.
        ...
        :param sub_scripts_dir: a directory where the intermediate scripts to be
                                generated to.
        :param prepend: A command to be pre-appended to the commands in the output
                       scripts (e.g., for singularity or docker)
        :param cmds_sh_file: A custom commands list file. Normally not used.
        :param module_load: Module loads within the sbatch submission scripts. If None
                            them ignored.
        ...
        sbatch_config_dict["pbprocesstools"]["sbatch"]["logfileout"] = os.path.join(
            log_dirs, "{}_log.out".format(jobname)
        )
        sbatch_config_dict["pbprocesstools"]["sbatch"]["logfileerr"] = os.path.join(
            log_dirs, "{}_log.err".format(jobname)
        )
        sbatch_config_dict["pbprocesstools"]["sbatch"]["time"] = job_time_limit
        sbatch_config_dict["pbprocesstools"]["sbatch"]["mem_per_core_mb"] = "{}".format(
            mem_per_core_mb
        )
        sbatch_config_dict["pbprocesstools"]["sbatch"]["ncores"] = "{}".format(
            n_cores_per_job
        )
        ...
        with open(run_script, "w") as run_script_file:
            ...
            run_script_file.write(
                "# Check the scripts directory exists, if not then create\n"
            )
            ...
            run_script_file.write(
                "# Split the input commands into a file for each job.\n"
            )
            sub_cmds_sh_file = os.path.join(sub_scripts_dir, "cmds_sh_file.sh")
            if prepend is None:
                run_script_file.write(
                    "splitcmdslist.py -i {} -o {} -f {} --dealsplit\n".format(
                        cmds_sh_file, sub_cmds_sh_file, n_jobs
                    )
                )
            else:
                run_script_file.write(
                    "{} splitcmdslist.py -i {} -o {} -f {} --dealsplit\n".format(
                        prepend, cmds_sh_file, sub_cmds_sh_file, n_jobs
                    )
                )
            run_script_file.write("\n")
            ...
            if prepend is None:
                run_script_file.write(
                    "genslurmsub.py -c {} -t {} -i {} -f {} -o {} --multi\n".format(
                        sbatch_config,
                        sbatch_template,
                        cmds_filelst,
                        cmds_srun,
    ...
    def run_gen_commands(self):
        """
        An abstract function which needs to be implemented with the functions and
        inputs you want run to generate the various commands and scripts to be
        executed.
    ...
    def run_check_outputs(self):
        """
        An abstract function which needs to be implemented with the functions and
        inputs you want run to check the outputs of the processing have been
